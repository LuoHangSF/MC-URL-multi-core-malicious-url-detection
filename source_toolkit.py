# only for source.py use, do not run it at any time
...
def single_test_core_v2(url, bayes, gbc, ada, rf, dt, lgs):
...
def batch_vote_test(test_csv_path, vote_func=mean_vote):
    ...
    for url in tqdm(urls, desc="Voting Test"):
        ...
        vote, _ = single_test_core_v2(url, bayes, gbc, ada, rf, dt, lgs)
        y_pred.append(vote)
...
#     batch_vote_test("./data-special/merged_test_small.csv")

if __name__ == '__main__':
    # single_test01()
    batch_vote_test("./data-special/merged_test.csv")
