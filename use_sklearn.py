...
def multi_machine_learing_models(data_train, data_cv):
    ...
    model_dir = 'model/six_classifier_model'
    if not os.path.exists(model_dir):
        os.makedirs(model_dir)
        print(f"已创建模型文件夹: {model_dir}")
    filename_bayes = 'model/six_classifier_model/c_bayes.model'
    filename_LGB = 'model/six_classifier_model/c_LGB.model'
    filename_ada = 'model/six_classifier_model/c_ada.model'
    filename_rf = 'model/six_classifier_model/c_rf.model'
    filename_decision_tree = 'model/six_classifier_model/c_decision_tree.model'
    filename_lgs = 'model/six_classifier_model/c_lgs.model'

    vote = []
    for i in range(len(y_test)):
        vote.append(0)

    bayes = BernoulliNB() # 使用伯努利朴素贝叶斯分类器
    bayes.fit(X_train, y_train) # 训练模型
    print('\nbayes模型的准确度:', bayes.score(X_test, y_test)) # bayes.
...
def loadModel() -> Tuple[Any, Any, Any, Any, Any, Any]:
    # 定义模型文件路径
    # filename_bayes = 'classifier_model\\c_bayes.model'  # 注意Windows路径用双反斜杠或正斜杠
    ...
    if os.path.exists(filename_bayes):
        ...
        raise FileNotFoundError(f"贝叶斯模型文件不存在: {filename_bayes}")

    if os.path.exists(filename_LGB):
        gbc = joblib.load(filename_LGB)
        print('成功读取梯度提升树模型！')
    else:
        raise FileNotFoundError(f"梯度提升树模型文件不存在: {filename_LGB}")

    if os.path.exists(filename_ada):
        ada = joblib.load(filename_ada)
        print('成功读取AdaBoost模型！')
    else:
        raise FileNotFoundError(f"AdaBoost模型文件不存在: {filename_ada}")

    if os.path.exists(filename_rf):
        rf = joblib.load(filename_rf)
        print('成功读取随机森林模型！')
    else:
        raise FileNotFoundError(f"随机森林模型文件不存在: {filename_rf}")

    if os.path.exists(filename_decision_tree):
        dt = joblib.load(filename_decision_tree)
        print('成功读取决策树模型！')
    else:
        raise FileNotFoundError(f"决策树模型文件不存在: {filename_decision_tree}")

    if os.path.exists(filename_lgs):
        lgs = joblib.load(filename_lgs)
        print('成功读取逻辑回归模型！')
    else:
        raise FileNotFoundError(f"逻辑回归模型文件不存在: {filename_lgs}")

    # 确保所有模型都加载成功
    if None in [bayes, gbc, ada, rf, dt, lgs]:
        raise ValueError("部分模型加载失败，请检查模型文件")
...
def extract_single_url_features(url, label, colnames):
    # 构造与批量处理一致的DataFrame
    df = pd.DataFrame({'URL': [url], 'label': [label]})
    # 用批量特征提取函数处理
    ...
    # print("=======================================")
    features['hostname_entropy'] = features['hostname_entropy'].astype(float)
    # print(features['hostname_entropy'])
    # print("=======================================")
    # print(features['hostname_entropy'].dtype)
    # print("=======================================")
    
    features.to_csv("test_single_01.csv", index=False, encoding='utf-8')
    # # 补齐所有特征列，顺序与colnames一致
    # for col in colnames:
...
def vote_to_predict_single_proba(url,label,colnames):
    features = extract_single_url_features(url, label, colnames)
    X = features.drop(['URL', 'label'], axis=1)
    
    bayes, gbc, ada, rf, dt, lgs = loadModel()
    vote = 0

    # 1. 贝叶斯分类器
    proba_bayes = bayes.predict_proba(X)
    print('proba_bayes:', proba_bayes)

    # 2. 梯度提升树分类器
    proba_ada = ada.predict_proba(X)
    print('proba_ada:', proba_ada)

    # 3. 随机森林分类器
    proba_rf = rf.predict_proba(X)
    print('proba_rf:', proba_rf)

    # 4. 决策树分类器
    proba_dt = dt.predict_proba(X)
    print('proba_dt:', proba_dt)

    # 5. 逻辑回归分类器
    proba_lgs = lgs.predict_proba(X)
    print('proba_lgs:', proba_lgs)

    # 6. 梯度提升树分类器
    proba_gbc = gbc.predict_proba(X)
    print('proba_gbc:', proba_gbc)
    
    return proba_bayes, proba_ada, proba_rf, proba_dt, proba_lgs, proba_gbc

def vote_to_predict_single_proba_v2(url,label,colnames, bayes, gbc, ada, rf, dt, lgs):
    features = extract_single_url_features(url, label, colnames)
    X = features.drop(['URL', 'label'], axis=1)
...
def eval_six_models(X_test, y_test):
    """
    分别评估六个模型的 Accuracy、Precision、Recall、F1-score
    """
    bayes, gbc, ada, rf, dt, lgs = loadModel()
    models = {
        "Bayes": bayes,
