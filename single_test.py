...
# 加权投票函数
def weighted_vote(proba_bayes, proba_ada, proba_rf, proba_dt, proba_lgs, proba_gbc, proba_tf_idf, proba_URLGledac):
    # 权重字典
    ...
    # weights = {"bayes": 1, "ada": 2, "rf": 20, "dt": 2, "lgs": 1, "gbc": 10, "tf_idf": 6, "URLGledac": 6}
    ...
    if sum(proba_good) > sum(prba_bad):
        return 0, sum(proba_good)
    else:
        return 1, sum(prba_bad)
...
def single_test_core(url):
    ...
    print("proba_tf_idf:", proba_tf_idf)
    print("proba_URLGledac:", proba_URLGledac)
...
def single_test():
    ...
    while(1):
        print("\n")
        print("============ single url detect ============")
        url = input("请输入要检测的URL（输入可以exit退出程序）：")
        ...
        if vote == 1:
            print(f"检测结果：恶意URL，概率为 {mean_proba:.4f}")
        elif vote == 0:
            print(f"检测结果：良性URL，概率为 {mean_proba:.4f}")
        else:
